...
void TCanvas::setScale(double scale)
{
    m_imageScale = scale;
    update();
}
...
{
    // 1. Смасштабировать исходное изображение
    ...
    // 2. Вычислить смещение рамки относительно начальной точки изображения
    ...
    // 3. Сохранить регион исходного изображения, ограниченный рамкой
...
void TCanvas::paintEvent(QPaintEvent*)
{
    ...
    emit frameMoved(m_frame->topLeft());
